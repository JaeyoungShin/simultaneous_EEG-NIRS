...
for vp = 1 : length(subdir_list)
    ...
    % marker selection: session (ssn)
    mrk_ssn = mrk_selectClasses(mrk_dsr, 'session');
       
    % segmentation: session (ssn)
    ssn = proc_segmentation(cnt_dsr, mrk_ssn, ival_epo); % 0-back
    
    % manipulation for EEGLAB compatible format
    ssn.x1 = permute(ssn.x, [2, 1, 3]); % nch x nsamples x ntrials
    
    for ch = 1 : length(mnt_dsr.clab)-2 % exclude two EOG channels
        chdata = ssn.x1(ch,:,:);
        
        [ersp(:,:,ch,vp),itc,powbase,times,freqs,erspboot,itcboot] =...
            newtimef(chdata, size(ssn.x1,2), ival_epo, ssn.fs, cycles,...
...
for ch = 1 : length(clab)-2
    ...
    imagesc(times/1000,freqs,ave_dsr(:,:,ch), clim); title(clab{ch});
